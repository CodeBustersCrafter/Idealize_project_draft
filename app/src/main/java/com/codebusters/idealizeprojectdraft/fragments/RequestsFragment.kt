...
class RequestsFragment(uid : String) : Fragment() {
    ...
    fun listenForUpdates(){
        firestore.collection(myTags.users).document(userID).collection(myTags.userRequests).addSnapshotListener { querySnapshot, _ ->
            if (querySnapshot != null) {
                // Process the updated data (e.g., convert to a list)
                for(document in querySnapshot){
                    firestore.collection(myTags.ads).document(document.get(myTags.requestAdID).toString()).get().addOnSuccessListener{
                            documentSnapshot->
                        firestore.collection(myTags.users).document(documentSnapshot.get(myTags.adUser).toString()).get().addOnSuccessListener{
                                documentSnapshotUser->
                            dataList.add(ModelBuilder().getRequestItem(document,documentSnapshot,documentSnapshotUser))
                            recyclerView.adapter?.notifyDataSetChanged()
                        }
    ...
    private fun listenForDeletions() {
        firestore.collection(myTags.users).document(userID).collection(myTags.userRequests).addSnapshotListener { querySnapshot: QuerySnapshot?, _ ->
